...
function createAccount(suri: string, pairType: KeypairType, { genesisHash, name, tags = [] }: CreateOptions, password: string, success: string, changeAccount: (account: string) => void, setValue: (value: string) => void): ActionStatus {
  ...
  const _onCommit = useCallback(
    (): void => {
      ...
      setTimeout((): void => {
        ...
        const status = createAccount(`${seed}${derivePath}`, pairType, options, password, t<string>('created account'), changeAccount, setValue);
